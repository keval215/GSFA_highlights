...
- The **`thor`** agent (reader / shared-context; replaces the former `docs-maintainer`) owns both of the above and also answers architecture questions directly (read-only) without invoking either command. See **[AGENTS.md](AGENTS.md)** for the full four-agent fleet (`ironman` orchestrator, `hawkeye` research, `thor` reader, `captain` coder).
